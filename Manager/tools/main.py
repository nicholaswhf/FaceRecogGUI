    ...
    def create_db(self):
        try:
            ...
            if (db_text.replace(' ','') != '') and (db_action is True):
                ...
                # 创建一个数据库表
                #res=query.exec_("create table record(ID int(4) , "
                         "Year varchar(4), Month varchar(2), Day varchar(2),Time varchar(8), Position int, Path varchar(50))")
                #print(res)
                # 插入三条数据
                #res=query.exec_("insert into record values(1000, '2019', '04','21','15:23:30',0,'./record/2019-04-21-15-23-30.jpg')")
