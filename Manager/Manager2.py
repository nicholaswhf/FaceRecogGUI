...
class TopFrame(QFrame):
    ...
    def initUi(self):
        ...
        self.btn_1=QPushButton(qtawesome.icon('fa.bug',color='#5691d3'),"考勤管理",self)
        ...
        '''
        self.btn_3=QPushButton(qtawesome.icon('fa.adjust',color='#5691d3'),"成员管理",self)
        self.btn_3.setObjectName("Visit")
        self.btn_3.move(500,0)
...
class TextFrame(QFrame):
    ...
    def add(self):
        ...
        if(len(self.idvalue.text())==0):
            ...
            return 
        out_dir="../TrainData/"+str(self.idvalue.text()+"/")
    ...
    def train2(self):
        res=os.system('python ../../openface/util/align-dlib.py ../traindata align outerEyesAndNose ../traindata-Aligned')
        if(res!=0):
            return
        if(os.path.exists("../TrainData-Aligned/cache.t7")):
            os.system('rm ../TrainData-Aligned/cache.t7')
        res=os.system('luajit ../../openface/batch-represent/main.lua -outDir ../Train-Features -data ../TrainData-Aligned')
        if(res!=0):
            return
        res=os.system('python ../../openface/demos/classifier.py train ../Train-Features')

    def train(self):

        res=subprocess.call('python ../../openface/util/align-dlib.py ../TrainData align outerEyesAndNose ../TrainData-Aligned',shell=True)
        
        if(res!=0):
            return
        if(os.path.exists("../TrainData-Aligned/cache.t7")):
            os.system('rm ../TrainData-Aligned/cache.t7')
        res=subprocess.call('luajit ../../openface/batch-represent/main.lua -outDir ../Train-Features -data ../TrainData-Aligned',shell=True)

        if(res!=0):
            return
        res=subprocess.call('python ../../openface/demos/classifier.py train ../Train-Features',shell=True)
