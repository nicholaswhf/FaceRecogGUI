...
class MainWindow(QMainWindow):
    ...
    def tolookupmember(self):
        ...
        self.QueryMember.update()
    def torecord(self):
        self.splitter1.widget(1).setParent(None)
        self.splitter1.insertWidget(1,self.QueryRecord)
        self.QueryRecord.update()
    def tovisitor(self):
        self.splitter1.widget(1).setParent(None)
        self.splitter1.insertWidget(1,self.QueryRecord)
...
class TextFrame(QFrame):
    ...
    def train(self):
        align.alignMain("./TrainData","TrainData-Aligned")
        if(os.path.exists("./TrainData-Aligned/cache.t7")):
            os.system('rm ./TrainData-Aligned/cache.t7')
        res=os.system('luajit ./Represent/main.lua -outDir ./Train-Features -data ./TrainData-Aligned -model ./Represent/nn4.small2.v1.t7')
        if(res!=0):
            print("[ERROR]represent false")
            return
        classifier.train("./Train-Features",'LinearSvm')
        msg="success"
        reply = QMessageBox.information(self,"Information",msg)
...
class RecordFrame(QFrame):
    def __init__(self):
        ...
        print(res)
        self.setFixedSize(650,388)
        #self.table_widget.setFixedSize(630,300)
        self.setObjectName("Text")
        print(self.model)
        self.table_widget.setAlternatingRowColors(1);
        self.fileslist=[] 
        ...
        self.deletebutton.move(15,300)
        self.deletebutton.setFixedSize(49,21) 
        self.deletebutton.clicked.connect(self.delete_record)  

        self.findbutton=QPushButton("查询",self)
        self.findbutton.setObjectName("FindButton")
        self.findbutton.move(80,300)
        self.findbutton.setFixedSize(49,21) 
        self.findbutton.clicked.connect(self.quert)
    ...
    def quert(self):
    	...
    	self.model.setFilter(("Day = "+value))
    	self.model.select()
