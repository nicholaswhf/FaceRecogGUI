...
class AlignDlib:
	...
	def align(self,imgDim,rgbImg,bb=None,landmarks=None,landmarkIndices=INNER_EYES_AND_BOTTOM_LIP,skipMulti=False):
		...
		print(npLandmarkIndices,len(npLandmarks))

		cv2.imshow("1ut",rgbImg)
		print("min,max",MINMAX_TEMPLATE)
		...
		#thumbnail=cv2.warpAffine(rgbImg,H,(rows,cols))
		return thumbnail


if __name__ == '__main__':
	align = AlignDlib("./openface/shape_predictor_68_face_landmarks.dat")
	rgb=cv2.imread("./openface/big/big2.bmp")
	print("######")
	outRgb = align.align(96, rgb)
	cv2.imshow("out",outRgb)
	cv2.waitKey(10000)
