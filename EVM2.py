...
#save video to files
def save_video(video_tensor,path="./real_diy.avi"):
    print("HERE:saving")
    fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
    [height,width]=video_tensor[0].shape[0:2]
    writer = cv2.VideoWriter(path, fourcc, 30, (width, height), 1)
    for i in range(0,video_tensor.shape[0]):
...
#manify motion
def magnify_motion(video_name,low,high,levels=3,amplification=2):
    t,f=load_video(video_name)
    start=time.time()
...
if __name__=="__main__":
    ...
    #magnify_motion("./openface/img/real/client009_session01_webcam_authenticate_adverse_1.mov",1,1.6)
    magnify_motion("./frameR.avi",1,1.6)
    '''
    cap=cv2.VideoCapture(0)
    while (1):
    	frame,ret=cap.read()

    #magnify_motion("./openface/img/real/testA.mp4",1,1.6)  
    '''